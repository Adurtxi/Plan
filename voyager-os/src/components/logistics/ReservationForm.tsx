...
export const ReservationForm: React.FC<ReservationFormProps> = ({ isOpen, onClose, editTarget }) => {
  ...
  return (
    <div className="fixed inset-0 z-[5000] flex justify-end bg-nature-primary/20 backdrop-blur-sm transition-all text-sm">
      <div className="w-full md:w-[600px] h-full bg-white shadow-2xl flex flex-col animate-[slideInRight_0.3s_ease-out]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-white z-10 shrink-0">
          ...
          <button onClick={onClose} className="p-2 text-gray-400 hover:bg-gray-100 hover:text-nature-primary rounded-full transition-colors">
            <X size={24} />
          </button>
        </div>

        {/* Scrollable Form */}
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scroll bg-nature-bg">

          {/* Type Selection */}
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 block">Tipo de Reserva</label>
              ...
              {TYPES.map(t => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold border transition-all ${active ? 'bg-nature-mint text-nature-primary border-nature-primary' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
                  >
          ...
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1 block">Título principal *</label>
              <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder={type === 'flight' ? 'Vuelo IB3434 a Madrid' : type === 'hotel' ? 'Hotel Riu Plaza' : 'Actividad...'} className="w-full p-3 bg-gray-50 rounded-xl focus:bg-white focus:ring-2 focus:ring-nature-primary outline-none transition-all border border-gray-200" />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1 block">Proveedor / Compañía</label>
                <input type="text" value={company} onChange={e => setCompany(e.target.value)} placeholder="Ej: Iberia, Booking.com..." className="w-full p-3 bg-gray-50 rounded-xl focus:bg-white focus:ring-2 focus:ring-nature-primary outline-none transition-all border border-gray-200" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1 block">Ref. Reserva / PNR</label>
                <input type="text" value={bookingReference} onChange={e => setBookingReference(e.target.value)} placeholder="Ej: Y6T8K2" className="w-full p-3 bg-gray-50 rounded-xl focus:bg-white focus:ring-2 focus:ring-nature-primary outline-none transition-all border border-gray-200 uppercase font-mono" />
              </div>
            </div>
          </div>
          ...
          {/* Dates & Times */}
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
            <h3 className="font-bold text-nature-text border-b border-gray-100 pb-2 mb-4 flex items-center gap-2">
              <Calendar size={18} className="text-nature-primary" /> Horarios
            </h3>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1 block">{type === 'hotel' ? 'Día Check-in' : 'Día Salida / Inicio'}</label>
                <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full p-3 bg-gray-50 rounded-xl border border-gray-200" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1 block">Hora</label>
                <input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full p-3 bg-gray-50 rounded-xl border border-gray-200" />
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 pt-2">
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1 block">{type === 'hotel' ? 'Día Check-out' : 'Día Llegada / Fin'}</label>
                <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full p-3 bg-gray-50 rounded-xl border border-gray-200" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1 block">Hora</label>
                <input type="time" value={endTime} onChange={e => setEndTime(e.target.value)} className="w-full p-3 bg-gray-50 rounded-xl border border-gray-200" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <div className="flex items-center justify-between border-b border-gray-100 pb-2 mb-4">
              <h3 className="font-bold text-nature-text flex items-center gap-2">
            ...
            {attachments.length === 0 ? (
              <p className="text-xs text-center text-gray-400 py-4">Sube los PDFs o capturas de tu billete para tenerlos a mano.</p>
            ) : (
              <div className="space-y-2">
                {attachments.map((att, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-xl">
                    ...
                    <span className="flex-1 text-xs font-bold text-gray-700 truncate">{att.name}</span>
                    <button onClick={() => setAttachments(prev => prev.filter((_, idx) => idx !== i))} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg">
                      <Trash2 size={16} />
                    </button>
                  </div>
                ))}
