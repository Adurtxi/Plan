...
export const Sidebar = () => {
  const { activeTab, setActiveTab, locations, checklist, deleteLocation, deleteChecklistItem, showDialog, addToast, theme, toggleTheme } = useAppStore();
  ...
  return (
    <aside className="w-20 md:w-24 bg-white border-r border-nature-border hidden lg:flex flex-col items-center py-8 gap-8 z-30 shadow-soft shrink-0">
      ...
      <button onClick={() => setActiveTab('gallery')} className={`p-4 rounded-2xl transition-all duration-300 ${activeTab === 'gallery' ? 'bg-nature-mint/50 text-nature-primary shadow-sm' : 'text-gray-400 hover:text-nature-primary hover:bg-nature-bg'}`} title="Galería"><ImageIcon size={24} strokeWidth={1.5} /></button>
